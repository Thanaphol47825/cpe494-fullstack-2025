// From Core team - BaseModuleApplication with Sub-Routes and Sub-Module Loading
// This just an example
// Update your module then delete this files
// To call this U need to update modules.json too 
class HrApplication extends BaseModuleApplication {
  ...
  async renderCreateInstructor() {
    this.templateEngine.mainContainer.innerHTML = `
      <div class="hr-create-instructor">
        ...
        <form style="max-width: 600px; margin: 20px 0;">
          ...
          <div style="margin-top: 20px;">
            <button type="submit" style="background: #28a745; color: white; padding: 10px 20px; border: none; border-radius: 4px; margin-right: 10px;">Save Instructor</button>
            <a routerLink="hr/instructors" style="background: #6c757d; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">Cancel</a>
          </div>
        </form>
      </div>
    `
  ...
  async renderStudents() {
    // Check if StudentManager sub-module is loaded
    if (this.isSubModuleLoaded('StudentManager.js') && window.StudentManager) {
      ...
      this.templateEngine.mainContainer.innerHTML = `
        <div class="hr-students">
          ...
          <div id="studentManagerContainer">
            <!-- Enhanced student manager will be rendered here -->
          </div>
      ...
      // Use the enhanced student manager
      const container = document.getElementById('studentManagerContainer')
      studentManager.renderEnhancedStudentList(container)
    } else {
      // Fallback to simple list if sub-module not loaded
      this.renderSimpleStudentList()
    }
  }

  renderSimpleStudentList() {
    this.templateEngine.mainContainer.innerHTML = `
      <div class="hr-students">
        <h2>👨‍🎓 Student Management</h2>
  ...
  async renderCreateStudent() {
    this.templateEngine.mainContainer.innerHTML = `
      <div class="hr-create-student">
        <h2>➕ Add New Student</h2>
        <p>Student registration form...</p>
        
        <div style="margin-top: 20px;">
          <a routerLink="hr/students" style="color: #6c757d;">← Back to Students</a>
        </div>
      </div>
    `
  }
  ...
  async renderLeave() {
    if (this.isSubModuleLoaded('LeaveManager.js') && window.LeaveManager) {
      ...
      this.templateEngine.mainContainer.innerHTML = `
        <div class="hr-leave">
          ...
          <div style="margin: 15px 0;">
            ...
            <a routerLink="hr/leave/history" style="background: #007bff; color: white; padding: 8px 16px; text-decoration: none; border-radius: 4px; margin-left: 10px;">View History</a>
          </div>
          
          <div id="leaveManagerContainer">
            <!-- Enhanced leave manager will be rendered here -->
          </div>
      ...
      const container = document.getElementById('leaveManagerContainer')
      container.innerHTML = `
        <div style="background: #f8f9fa; padding: 20px; border-radius: 8px;">
          <h3>📊 Leave Status Overview</h3>
          <p>Enhanced leave management features are now available!</p>
          <p>Use the buttons above to create new leave requests or view your leave history.</p>
        </div>
      `
    } else {
      this.renderSimpleLeave()
    }
  }
  ...
  async renderCreateLeave() {
    if (this.isSubModuleLoaded('LeaveManager.js') && window.LeaveManager) {
      ...
      this.templateEngine.mainContainer.innerHTML = `
        <div class="hr-create-leave">
          ...
          <div id="leaveFormContainer">
            <!-- Enhanced leave form will be rendered here -->
          </div>
      ...
      leaveManager.renderLeaveRequestForm(container)
    } else {
      this.renderSimpleCreateLeave()
    }
  }

  renderSimpleCreateLeave() {
    this.templateEngine.mainContainer.innerHTML = `
      <div class="hr-create-leave">
        <h2>➕ Create Leave Request</h2>
    ...
    if (this.isSubModuleLoaded('LeaveManager.js') && window.LeaveManager) {
      ...
      this.templateEngine.mainContainer.innerHTML = `
        <div class="hr-leave-history">
          <h2>📊 Leave History</h2>
          <div id="leaveHistoryContainer">
            <!-- Enhanced leave history will be rendered here -->
          </div>
      ...
      leaveManager.renderLeaveHistory(container)
    } else {
      this.templateEngine.mainContainer.innerHTML = `
        <div class="hr-leave-history">
          <h2>📊 Leave History</h2>
          <p>Leave history functionality requires enhanced module.</p>
