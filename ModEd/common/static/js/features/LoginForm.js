class CommonLoginFormFeature {
  ...
  async render() {
    ...
    header.innerHTML = `
      ...
      <header style="margin-bottom: 24px;">
        <h2 style="font-size: 1.5rem; font-weight: 600; color: #2d2d2d;">Login</h2>
      </header>
    ...
    try {
      this.formRenderer = new AdvanceFormRender(this.templateEngine, {
        modelPath: "common/login",
        targetSelector: "#loginForm",
      ...
      console.log("✅ Login form rendered using AdvanceFormRender");
      return true;
    } catch (error) {
  ...
  async handleSubmit(formData, _event, formInstance) {
    this.showMessage("Saving login...", "info");
...
if (typeof window !== "undefined") {
  window.CommonLoginFormFeature = CommonLoginFormFeature;
}

console.log("📦 CommonLoginFormFeature loaded (using AdvanceFormRender)");