// Student Resignation List Feature using AdvanceTableRender (V2)
  ...
  class HrStudentResignationListFeature {
    ...
    async #createListPage() {
      try {
        ...
        const pageElement = helpers && helpers.createListPageLayout ? helpers.createListPageLayout({
          title: 'Student Resignation Requests',
          description: 'Manage student withdrawal and resignation requests',
          ...
          addButtonLink: 'hr/resignation/student/create',
    ...
    async #renderWithAdvanceTableRender(resignations) {
      ...
      const preparedData = resignations.map((resignation) => {
        ...
        return {
          ...
          submission_date_display: this.#formatDate(resignation.CreatedAt ?? resignation.created_at),
        ...
        },
        {
          ...
          template: `
            <div class="flex gap-2 justify-center">
              <button onclick="hrStudentResignationList.viewResignation('{request_id}')"
              ...
              <button onclick="hrStudentResignationList.editResignation('{request_id}')"
              ...
              <button onclick="hrStudentResignationList.deleteResignation('{request_id}', '{student_code}')"
    ...
    #renderResignationTable(resignations) {
      ...
      ['Request ID', 'Student Code', 'Status', 'Submitted At', 'Actions'].forEach((header) => {
      ...
      resignations.forEach((resignation) => {
        ...
        const cells = [
          ...
          { content: studentCode, className: 'px-6 py-4 whitespace-nowrap text-sm text-gray-900' },
          {
          ...
          { content: formatDate(resignation.CreatedAt ?? resignation.created_at), className: 'px-6 py-4 whitespace-nowrap text-sm text-gray-500' }
        ];
        ...
        const viewBtn = helpers ? helpers.createActionButton('view', null, requestId, 'View') : this.#createActionButton('View');
        viewBtn.onclick = () => this.viewResignation(requestId);

        const editBtn = helpers ? helpers.createActionButton('edit', null, requestId, 'Edit') : this.#createActionButton('Edit', 'yellow');
        editBtn.onclick = () => this.editResignation(requestId);

        const deleteBtn = helpers ? helpers.createActionButton('delete', null, requestId, 'Delete') : this.#createActionButton('Delete', 'red');
        deleteBtn.onclick = () => this.deleteResignation(requestId, studentCode);
    ...
    async viewResignation(resignationId) {
      try {
        ...
        const existing = document.getElementById('studentResignationDetailModal');
        if (existing) existing.remove();

        const overlay = document.createElement('div');
        overlay.id = 'studentResignationDetailModal';
        ...
        header.innerHTML = `
          <div>
            ...
            <p class="text-sm text-slate-500">Student ${resignation.StudentCode || resignation.student_code || 'N/A'}</p>
          </div>
        ...
        const fields = [
          { label: 'Student Code', value: resignation.StudentCode || resignation.student_code || 'N/A' },
    ...
    async deleteResignation(resignationId, studentCode) {
      ...
      try {
        await this.api.deleteStudentResignation(resignationId);
    ...
    #showError(message) {
      this.templateEngine.mainContainer.innerHTML = HrTemplates.render('errorPage', {
        ...
        backLabel: 'Back to Resignation Menu'
      });
    }

    #exposeGlobally() {
      window.hrStudentResignationList = this;
      window.studentResignationList = this;
    }
  }
