// Instructor Resignation Edit Feature using DOM helpers
if (typeof HrInstructorResignationEditFeature === 'undefined') {
  class HrInstructorResignationEditFeature {
    ...
    async render() {
      ...
      try {
        ...
        console.error('Error rendering instructor resignation edit:', error);
    ...
    async #loadResignation() {
      this.resignationData = await this.api.fetchInstructorResignation(this.requestId);
    }

    async #renderEditForm() {
      const pageWrapper = HrUiComponents.createEditFormPageWrapper({
        title: 'Edit Instructor Resignation',
        description: `Request #${this.requestId}`,
        icon: HrUiComponents.iconPaths?.instructor || HrUiComponents.iconPaths.edit,
        gradientFrom: 'orange-600',
        gradientTo: 'red-600',
        bgGradient: 'from-orange-50 via-amber-50 to-yellow-50',
        formTitle: 'Resignation Details',
        containerSelector: '.instructor-resignation-edit-container',
        backLink: 'hr/resignation/instructor',
      ...
      const formContainer = pageWrapper.querySelector('.instructor-resignation-edit-container');
      ...
      this.formRender = new window.AdvanceFormRender(this.templateEngine, {
        ...
        targetSelector: '.instructor-resignation-edit-container',
    ...
    async #handleSubmit(formData) {
      ...
      try {
        const result = await this.api.updateInstructorResignation(this.requestId, payload);
        HrUiComponents.showFormSuccess('Resignation updated successfully!', result);
        setTimeout(() => {
          window.location.href = '#hr/resignation/instructor';
        }, 2000);
      } catch (error) {
        console.error('Instructor resignation update error:', error);
        HrUiComponents.showFormError(error.message || 'Failed to update resignation request', error);
      }
    }

    #validateFormData(formData) {
      const missing = ['InstructorCode', 'Reason', 'Status'].filter((field) => !formData[field]);
      if (missing.length > 0) {
    ...
    #getInitialFormData() {
      return {
        ...
        InstructorCode: this.#getValue('InstructorCode', 'instructor_code'),
    ...
    #attachActionButtons() {
      const form = document.querySelector('.instructor-resignation-edit-container form');
      if (!form) return;
    ...
    #showError(message) {
      ...
      if (helpers && typeof helpers.createErrorPage === 'function') {
        const errorPage = helpers.createErrorPage({
          message,
          backUrl: `${this.rootURL}/hr/resignation/instructor`,
          ...
          <div class="max-w-4xl mx-auto px-4">
            <div class="bg-red-50 border border-red-200 rounded-lg p-6">
              ...
              <div class="mt-4 flex gap-3">
                ...
                <button onclick="window.location.href='#hr/resignation/instructor'" class="${HrUiComponents.buttonClasses.secondary}">
                  Back to Requests
                </button>
              </div>
            </div>
  ...
  if (typeof window !== 'undefined') {
    window.HrInstructorResignationEditFeature = HrInstructorResignationEditFeature;
  }
}
