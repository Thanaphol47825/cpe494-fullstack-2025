class InterviewManage {
  ...
  async render() {
    ...
    document.getElementById('createBtn').onclick = () => this.showCreateForm();
    document.getElementById('setupMockDataBtn').onclick = () => this.setupMockData();
    await this.load();
  }
  ...
  async load() {
    ...
    try {
      const res = await fetch((window.__ROOT_URL__ || 'http://localhost:8080') + '/recruit/GetInterviews');
      const data = await res.json();
      if (!data.isSuccess) throw new Error(data.result || 'Failed to load interviews');
      ...
      if (interviews.length === 0) {
        ...
        document.getElementById('createFirstBtn').onclick = () => this.showCreateForm();
        status.textContent = 'No interviews found';
        return;
      }

      const rows = interviews.map(item => {
        const statusColor = this.getStatusColor(item.interview_status);
        const scheduledDate = item.scheduled_appointment ? new Date(item.scheduled_appointment).toLocaleDateString() : 'Not set';
        const totalScore = item.total_score || 0;
        
       
        const id = item.ID || item.id;
        const instructorId = item.instructor_id;
        const applicationReportId = item.application_report_id;
        
        return `
          <tr class="border-b hover:bg-gray-50">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${id}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${instructorId}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${applicationReportId}</td>
            <td class="px-6 py-4 whitespace-nowrap">
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColor}">
                ${item.interview_status || 'Not set'}
              </span>
            </td>
      ...
      wrap.querySelectorAll('.viewBtn').forEach(btn => btn.onclick = (e) => this.view(e.target.dataset.id));
      wrap.querySelectorAll('.editBtn').forEach(btn => btn.onclick = (e) => this.edit(e.target.dataset.id));
      wrap.querySelectorAll('.delBtn').forEach(btn => btn.onclick = (e) => this.remove(e.target.dataset.id));
  ...
  async view(id) {
    try {
      const res = await fetch((window.__ROOT_URL__ || 'http://localhost:8080') + '/recruit/GetInterview/' + id);
      const data = await res.json();
      if (!data.isSuccess) throw new Error(data.result || 'Failed to load interview');
      ...
      const instructorId = interview.instructor_id;
      const applicationReportId = interview.application_report_id;
  ...
  async edit(id) {
    try {
          ...
          <form id="editForm" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="instructor_id" value="${instructorId}" required
                       class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700">Application Report ID</label>
                <input type="number" name="application_report_id" value="${applicationReportId}" required
                       class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
              </div>
            </div>
      ...
      document.getElementById('editForm').onsubmit = (e) => this.submitEdit(e, id);
    } catch (err) {
      alert('Error loading interview: ' + err.message);
    }
  }
  ...
  async submitEdit(e, id) {
    ...
    try {
      const res = await fetch((window.__ROOT_URL__ || 'http://localhost:8080') + '/recruit/UpdateInterview/' + id, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      });
      const result = await res.json();
      if (!result.isSuccess) throw new Error(result.result || 'Update failed');
  ...
  async remove(id) {
    ...
    try {
      const res = await fetch((window.__ROOT_URL__ || 'http://localhost:8080') + '/recruit/DeleteInterview/' + id, { 
        method: 'POST' 
      });
      const data = await res.json();
      if (!data.isSuccess) throw new Error(data.result || 'Delete failed');
  ...
  async setupMockData() {
    const status = document.getElementById('status');
    status.textContent = 'Setting up mock data...';
    status.className = 'text-sm text-gray-500';
    
    try {
      const res = await fetch((window.__ROOT_URL__ || 'http://localhost:8080') + '/recruit/SetupMockData', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      const data = await res.json();
      ...
      if (data.isSuccess) {
        status.textContent = 'Mock data created successfully!';
        status.className = 'text-sm text-green-600';
        await this.load(); 
      } else {
        status.textContent = 'Failed to setup mock data';
        status.className = 'text-sm text-red-600';
      }
    } catch (err) {
      status.textContent = 'Error setting up mock data';
      status.className = 'text-sm text-red-600';
    }
  }
