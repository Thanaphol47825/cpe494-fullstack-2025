if (typeof window !== 'undefined' && !window.ApplicationRoundService) {
  class ApplicationRoundService {
    ...
    async getById(id) {
      ...
      try {
        ...
        const applicationRound = payload?.result || payload || {};
        return { success: true, data: applicationRound };
      } catch (err) {
        return { success: false, error: err.message };
      }
    }
  ...
  window.ApplicationRoundService = ApplicationRoundService;
}
