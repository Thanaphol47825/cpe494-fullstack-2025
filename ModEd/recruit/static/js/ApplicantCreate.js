...
class RecruitApplication {
  ...
  async render() {
    ...
    this.application.mainContainer.innerHTML = "<h2>Recruit Module</h2>";
    ...
    for (const module of this.subModules) {
      ...
      button.className = "inline-block mt-2 text-blue-600 underline cursor-pointer";

      const ModuleClass = window[module.className];
      if (!ModuleClass) {
        console.error(`Module ${module.className} not found`);
        continue;
      }
      const moduleEngine = new ModuleClass(this.application);
