...
type Curriculum struct {
	...
	// DepartmentId uint               `json:"DepartmentId" gorm:"not null" csv:"department_id" form:"label:Department;type:select;required:true;apiurl:/api/data/common/departments/getall"`
	Department  model.Department   `json:"Department" gorm:"foreignKey:DepartmentId;references:ID" csv:"-" form:"-"`
	ProgramType *model.ProgramType `json:"ProgramType" gorm:"type:int;not null" csv:"program_type" form:"label:Program Type;type:select;apiurl:/curriculum/Curriculum/getProgramTypeOptions"`
