if (typeof window !== "undefined" && !window.CompanyList) {
  class CompanyList {
    ...
    async #createListPage() {
      try {
        ...
        try {
          ...
          console.warn(
            "AdvanceTableRender failed, falling back to manual render:",
            error
          );
    ...
    #createListPageLayout() {
      ...
      container.className =
        "min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-8";
      ...
      headerCard.className =
        "bg-gradient-to-r from-blue-600 to-purple-600 rounded-3xl shadow-2xl p-8 text-white";
      ...
      description.textContent =
        "Manage internship companies and their information";
      ...
      backButton.className =
        "inline-flex items-center px-6 py-3 bg-white/20 backdrop-blur-sm text-white rounded-xl hover:bg-white/30 transition-all duration-200 shadow-lg";
      ...
      cardHeader.className =
        "bg-gradient-to-r from-blue-500 to-purple-600 px-8 py-6 flex items-center justify-between";
      ...
      addButton.className =
        "inline-flex items-center px-6 py-3 bg-white text-blue-600 rounded-xl hover:bg-blue-50 transition-all duration-200 shadow-lg font-semibold";
      addButton.onclick = () =>
        (window.location.href = "#internship/company/create");
    ...
    async #renderWithAdvanceTableRender(companies) {
      ...
      const preparedData = companies.map((company) => ({
        ...
        company_address:
          company.company_address || company.CompanyAddress || "N/A",
      }));
    ...
    #renderCompanyTable(companies) {
      ...
      companies.forEach((company) => {
        ...
        nameCell.textContent =
          company.company_name || company.CompanyName || "N/A";
        ...
        // Delete button
        const companyName =
          company.company_name || company.CompanyName || "Unknown";
        const deleteBtn = this.#createDeleteButton(
          company.ID || company.id,
          companyName
        );
        deleteBtn.onclick = () =>
          this.deleteCompany(company.ID || company.id, companyName);
    ...
    #createEditButton(id) {
      ...
      button.className =
        "inline-flex items-center px-3 py-1.5 bg-yellow-50 text-yellow-700 text-sm rounded-lg hover:bg-yellow-100 transition-colors";
      button.type = "button";
    ...
    #createDeleteButton(id, name) {
      const button = document.createElement("button");
      button.className =
        "inline-flex items-center px-3 py-1.5 bg-red-50 text-red-700 text-sm rounded-lg hover:bg-red-100 transition-colors";
      button.type = "button";
    ...
    async deleteCompany(id, name) {
      ...
      try {
        const response = await fetch(
          `${this.rootURL}/curriculum/company/delete/${id}`,
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            error?.message || `Delete failed (${response.status})`
          );
        }
