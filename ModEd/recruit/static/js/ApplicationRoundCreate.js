class ApplicationRoundCreate {
  constructor(engine, rootURL, roundId = null) {
    this.engine = engine;
  ...
  async render() {
    ...
    try {
      await this.#createDynamicRoundForm();
      if (this.roundId != null) await this.#prefillExisting(this.roundId);
      return true;
    } catch (err) {
      console.error(err);
  ...
  async #createDynamicRoundForm() {
    // 1) load schema for ApplicationRound
    const metaURL = this.rootURL + "/api/modelmeta/applicationround";
    ...
    // 2) ensure template is ready (same pattern as ApplicantList_Create)
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="text-center mb-12">
            ...
            <h1 class="text-4xl font-bold text-gray-900 mb-2">
              ${this.roundId != null ? "Edit Application Round" : "Create Application Round"}
            </h1>
          ...
          <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
            ...
            <div class="p-8">
              <div class="round-form-container"></div>
    ...
    // 4) render fields via FormRender
    const formRender = new FormRender(application, this._schema, ".round-form-container");
    await formRender.render();

    // 5) style + require + hooks
    ...
    const container = this.engine.mainContainer.querySelector(".round-form-container");
  ...
  async #prefillExisting(id) {
    try {
      const res = await fetch(`${this.rootURL}/recruit/GetApplicationRound/${id}`);
      const data = await res.json();
      if (!data?.isSuccess) return;

      const f = this.engine.mainContainer.querySelector(".round-form-container form");
      if (!f) return;
  ...
  #applyTailwindStyles() {
    const root = this.engine.mainContainer.querySelector(".round-form-container");
    if (!root) return;
  ...
  #styleSubmitButton() {
    ...
    if (!submit.value) submit.textContent = this.roundId != null ? "Update" : "Create";
  }

  #enforceRequiredFields() {
    const form = this.engine.mainContainer.querySelector(".round-form-container form");
    if (!form) return;
  ...
  #buildSanitizedPayload(form) {
    ...
    if (this.roundId != null) data.id = Number(this.roundId);
    return data;
  }
  ...
  async #submit(e) {
    ...
    this.#setStatus(this.roundId != null ? "Updating..." : "Submitting...", "text-gray-600");
    submitBtn?.setAttribute("disabled", "disabled");

    const data = this.#buildSanitizedPayload(form);
    const url =
      this.rootURL +
      (this.roundId != null ? "/recruit/UpdateApplicationRound" : "/recruit/CreateApplicationRound");
    ...
    try {
      ...
      this.#setStatus(
        this.roundId != null ? "Updated successfully!" : "Application round created successfully!",
        "text-green-600"
      );
      ...
      if (this.roundId == null) form.reset();
    } catch (err) {
  ...
  async #goBack() {
    try {
      await this.engine.fetchModule("/recruit/static/js/ApplicationRoundList.js");
      const list = new ApplicationRoundList(this.engine, this.rootURL);
      this.engine.mainContainer.innerHTML = "";
      await list.render();
    } catch (e) {
