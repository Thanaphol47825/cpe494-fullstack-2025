...
if (typeof window !== "undefined" && !window.InternStudentSkillCreate) {
  class InternStudentSkillCreate {
    constructor(application, internStudentId = null, studentSkillId = null) {
      ...
      // ---------- ROLE ----------
      const rawRole =
        localStorage.getItem("internstudentskill_role") ||
        localStorage.getItem("role") ||
        "student";

      this.userRole = String(rawRole).toLowerCase();
      console.log("[InternStudentSkillCreate] role =", this.userRole);
    }
    ...
    preparePageConfig() {
      ...
      const description = descName
        ? `For: ${descName}`
        : "Link a student with a skill and set the level.";
    ...
    async loadData() {
      ...
      if (this.internStudentId) {
        const internResponse = await fetch(
          `/curriculum/InternStudent/${this.internStudentId}`
        );
    ...
    async generateFormFields() {
      ...
      const editStudentId = this.studentSkillData?.student_id
        ? String(this.studentSkillData.student_id)
        : "";
      const editSkillId = this.studentSkillData?.skill_id
        ? String(this.studentSkillData.skill_id)
        : "";
      const editLevel = this.studentSkillData?.level
        ? String(this.studentSkillData.level)
        : "1";

      const lockedStudentId = this.internStudentId
        ? String(this.internStudentId)
        : "";
      ...
      if (lockedStudentId) {
        ...
        const labelText = studentFullName
        ...
        const opts = studentOptions
          .map((o) => {
            const selectedValue =
              editStudentId || studentOptions[0]?.value || "";
            return `<option value="${o.value}" ${
              String(selectedValue) === o.value ? "selected" : ""
            }>${o.label}</option>`;
          })
          .join("");
        studentFieldHTML = `
      ...
      const skillOpts = skillOptions
        .map((o) => {
          const selectedValue = editSkillId || skillOptions[0]?.value || "";
          return `<option value="${o.value}" ${
            String(selectedValue) === o.value ? "selected" : ""
          }>${o.label}</option>`;
        })
      ...
      const levelOpts = levelOptions
        .map((o) => {
          const selectedValue = editLevel || "1";
          return `<option value="${o.value}" ${
            String(selectedValue) === o.value ? "selected" : ""
          }>${o.label}</option>`;
        })
    ...
    async handleSubmit(e) {
      ...
      const url = this.isEditMode
        ? this.endpoints.update(this.studentSkillId)
      ...
      try {
        ...
        if (!res.ok || !result.isSuccess) {
          throw new Error(
            result.error ||
              `Failed to ${this.isEditMode ? "update" : "create"} student skill`
          );
        }
